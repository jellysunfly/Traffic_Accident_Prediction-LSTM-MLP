...
data_indexes = np.arange(dataset_size)  # 0 ~ 50302 index 생성
...
# LSTM model (for proposed our model)
input_data = Input(shape=(4, 16))

lstm_1 = LSTM(units=64, return_sequences=True)(input_data)
lstm_2 = LSTM(units=32, return_sequences=False)(lstm_1)

result = Dense(units=1, activation='sigmoid')(lstm_2)
...
hist = LSTM_model.fit(X_train, Y_train, epochs=100, batch_size=32, validation_data=(X_val, Y_val), callbacks = [early_stopping], verbose=1)
